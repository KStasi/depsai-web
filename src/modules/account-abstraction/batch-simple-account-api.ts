...
export type TypeDataFreeTransactionDetails = Omit<
...
export class BatchAccountAPI extends SimpleAccountAPI {
   ...
   */
  async createSignedUserBatchOp(
    info: TransactionDetailsForUserBatchOp
   ...
   * @param info
   */
  async createUnsignedUserBatchOp(
    info: TransactionDetailsForUserBatchOp
  ): Promise<UserOperationStruct> {
  ...
  async encodeExecuteBatch(
    ...
    const I = new utils.Interface([
      'function executeBatch(address[] target, uint256[] value, bytes[] data) external'
    ]);
